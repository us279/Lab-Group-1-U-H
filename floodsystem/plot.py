...
def plot_water_level_with_fit(station, dates, levels, p):
    if dates == []:
        ...
        for i in stations:
            if i.name == station:
                typical_low=np.linspace(i.typical_range[0],i.typical_range[0],len(dates))
                typical_high=np.linspace(i.typical_range[1],i.typical_range[1],len(dates))
        
        #Plots
        plt.plot(dates,levels)
        plt.plot(dates, polynomial(np.array(matplotlib.dates.date2num(dates))-d0))
